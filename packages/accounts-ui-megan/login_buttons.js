...
validateEmail = function (email) {
  ...
  if (email.indexOf('@') !== -1) {
    ...
    loginButtonsSession.errorMessage("Invalid email address");
    return false;
  }
};
...
validatePassword = function (password) {
	...
	var nums = "0123456789";
	var errorMsgs = {};
	errorMsgs["length"] = "Error: Password must be at least 6 characters long.";
	errorMsgs["speChars"] = "Error: Password must contain a special character: " + speChars;
	errorMsgs["nums"] = "Error: Password must contain a number: " + nums;
	var errorMsg = "";
	
	//checks password length
    if (password.length < 6) {
		errorMsg += errorMsgs["length"];
    } 
	
	//checks for existence of special character
	var boolSpeChar = false;
	for(var i = 0; i < password.length; i++){
		if (speChars.indexOf(password.charAt(i)) >= 0){
			boolSpeChar = true;
		}
	}
	if (boolSpeChar == false){
		errorMsg += errorMsgs["speChars"];
	}
	
	//checks for existence of number
	var boolNum = false;
	for(var i = 0; i < password.length; i++){
		if (nums.indexOf(password.charAt(i)) >= 0){
			boolNum = true;
		}
	}
	if (boolNum == false){
		errorMsg += errorMsgs["nums"];
	}
	
	//allows password creation or prints error message array
	if (errorMsg == ""){
		return true
	}
	else {
		loginButtonsSession.errorMessage(errorMsg);
		return false;
    }
  };
